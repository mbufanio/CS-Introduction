...
- [ ] Test-run this folder's `lesson_demo.m` once (the Haunted School — today's
      backup game and going-away gift; its rooms are the `haunted_*.m` files)
- [ ] Type the current year into `predict_the_output.m`'s trophy banner (the `____`
...
Week 15's skeleton on the smartboard (`fill_in_the_blank.m` plus `room_1.m` …
...
### If the build finishes early · Polish round (`fill_in_the_blank.m`)
...
playthrough's wrap if you can. Completed versions in `answer-key.md`.

### 0:30–0:38 · The finale trivia (`predict_the_output.m`)
...
Open `challenge.m` — the showcase: a 1966-style chatbot the class can talk to
...
- **"Can we keep the game?"** Yes — that's the plan. This folder's `lesson_demo.m`
