...
- [ ] Print `step6_class_handout.md` (one per student), have `step2_teacher_answer_key.md` open or printed
- [ ] **Bring a real die** — a big foam one is ideal. Board space for a tally chart
- [ ] Dry-run `step3_class_demo.m` — the million-roll section takes a couple of seconds;
...
`week-12-turtle-art/step3_class_demo.m` Section 7 (set the `TWEAK THESE` dials to the
...
Open `step7_class_challenge.m`: roll-until-doubles (how many tries? re-run three times!), the
