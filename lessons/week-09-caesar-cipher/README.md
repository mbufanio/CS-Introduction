...
- [ ] Print `handout.pdf` (one per student — it has the cipher strip they decode with), have `answer-key.md` open or printed for you
- [ ] Open MATLAB, set the Current Folder to this lesson's folder, zoom fonts for the smartboard (see `TEACHER-GUIDE.md`), type `clc`
- [ ] Test-run `lesson_demo.m` once — make sure the brute-force finale scrolls readably at smartboard font size
...
Open `challenge.m`: ROT13, the shift that encodes and decodes with the SAME
