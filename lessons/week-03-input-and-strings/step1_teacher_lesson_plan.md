...
- [ ] Test-run `step3_class_demo.m` once — it has `input()` now, so YOU type answers in
...
Run the first section of `step3_class_demo.m`. The Command Window prints
...
Open `step7_class_challenge.m`: the MEGA Mad Libs (a number input does live math *inside* the
