...
- [ ] Print `step6_class_handout.pdf` (one per student — it has the mini ASCII table they'll need), have `step2_teacher_answer_key.md` open or printed for you
- [ ] Open MATLAB, set the Current Folder to this lesson's folder, zoom fonts for the smartboard (see `TEACHER-GUIDE.md`), type `clc`
- [ ] Test-run `step3_class_demo.m` once — each `%%` section has its own reveal
...
always. Completed code is in `step2_teacher_answer_key.md`.

### 0:30–0:37 · Voting game: `step5_class_predict_the_output.m`
...
Open `step7_class_challenge.m`: a spell-your-name-in-secret-numbers converter (run it once per
